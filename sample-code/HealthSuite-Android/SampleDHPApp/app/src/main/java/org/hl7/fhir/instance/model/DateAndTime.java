...
public class DateAndTime {
	...
	public DateAndTime(String xDate) throws ParseException {
		...
		try {
			...
			throw new ParseException("The date '"+xDate+"' is not a valid Date Time Format at character "+ Integer.toString(offset), offset);
		}
	}
	...
	private static int readField(String date, int i, int j) {
        ...
        // Added by HS, Integer.parseInt throws an exception for "+", so removing signs for now
        boolean negativeIn = s.startsWith("-");
        s = s.replaceAll("[\\+-]", "");

		return negativeIn ? -1*Integer.parseInt(s) : Integer.parseInt(s);
	}
	...
	public String toString() {
		...
		b.append(Utilities.padLeft(Integer.toString(year), '0', 4));
		if (month != 0) {
			b.append("-");
			b.append(Utilities.padLeft(Integer.toString(month), '0', 2));
			if (day != 0) {
				b.append("-");
				b.append(Utilities.padLeft(Integer.toString(day), '0', 2));
				if (time) {
					b.append("T");
					b.append(Utilities.padLeft(Integer.toString(hour), '0', 2));
					b.append(":");
					b.append(Utilities.padLeft(Integer.toString(minute), '0', 2));
					if (seconds) {
						b.append(":");
						b.append(Utilities.padLeft(Integer.toString(second), '0', 2));
						if (fractions > 0) {
							b.append(".");
							b.append(Utilities.padLeft(Integer.toString(fraction), '0', fractions));
						}
					}
				}
				if (timezone != null) {
					if (!timezone) {
						...
						if (tzHour > 0) {
							b.append("+");
							b.append(Utilities.padLeft(Integer.toString(tzHour), '0', 2));
						} else { 
							b.append("-");
							b.append(Utilities.padLeft(Integer.toString(-tzHour), '0', 2));
						}
						b.append(":");
						b.append(Utilities.padLeft(Integer.toString(tzMin), '0', 2));
					}
				}
			}
		}
	...
	public Calendar toCalendar() {
		...
		if (timezone == null) {
			...
			if (tzHour < 0)
			  tz = TimeZone.getTimeZone("GMT - "+Utilities.padLeft(Integer.toString(-tzHour), '0', 2)+" : "+ Integer.toString(tzMin, 2));
			else
			  tz = TimeZone.getTimeZone("GMT + "+Utilities.padLeft(Integer.toString(tzHour), '0', 2)+" : "+ Integer.toString(tzMin, 2));
	  	}
